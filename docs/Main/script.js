...
// pulls list of cities from local storage
...
// pulls the current city into local storage to display forecast on reload
...
// saves currently displayed city to local storage
function storeCurrentCity() {
  localStorage.setItem('currentCity', JSON.stringify(cityname));
}

// clears cities stored in local storage
...
// event handler for if user presses enter to search for a city
...
// runs weather API that calls for the current cities weather
async function displayWeather() {
  // CURRENT WEATHER API
  var queryURL =
    'https://api.openweathermap.org/data/2.5/weather?q=' +
    cityname +
    '&units=imperial&appid=d202bea0172cc5628ace1f64febee1fe';
  ...
  var currentCityEl = $("<h3 class = 'cardbody'>").text(
    getCurrentCity + '(' + val + ')'
  );
  ...
  // UV API
  var uvURL =
    'https://api.openweathermap.org/data/2.5/uvi?appid=49f0c9f2f3643047ff38e9feb907e820&lat=' +
    getLat +
    '&lon=' +
    getLong;
...
// runs 5 day forcast API
async function displayFiveDayForecast() {
  var queryURL =
    'https://api.openweathermap.org/data/2.5/forecast?q=' +
    cityname +
    '&units=imperial&appid=e380bf05f66de537690972a12823d83f';
...
// allows user to display weather for cities in history list by clicking city name
$(document).on('click', '.city', historyDisplayWeather);
// clear local storage/cities button
